...
export function useAdminModeration() {
  ...
  const fetchModerationEvents = useCallback(async (filters?: {
    dateRange?: 'today' | '7days';
    userId?: string;
    category?: string;
    severity?: string;
  }) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (filters?.category) {
          setEvents(eventsWithNicks.filter(e => 
  ...
  const liftSuspension = useCallback(async (userId: string) => {
    if (!isAdmin) return { error: new Error('No autorizado') };
