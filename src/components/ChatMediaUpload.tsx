...
export function ChatMediaUpload({
  ...
  return <>
      ...
      {/* Audio button */}
      <button onClick={handleAudioClick} disabled={disabled || isUploading} className="p-3 rounded-xl bg-card text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50">
      ...
      <AnimatePresence>
        {showAudioModal && createPortal(<>
          ...
          opacity: 0
        }} onClick={closeAudioModal} className="fixed inset-0 bg-black/50 z-[9999]" />
              <motion.div initial={{
          scale: 0.95,
          opacity: 0
        }} animate={{
          ...
          scale: 0.95,
          opacity: 0
        }} className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-card rounded-2xl p-6 z-[10000] w-11/12 max-w-sm" role="dialog" aria-modal="true">
                ...
                <button onClick={closeAudioModal} className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-medium">
                  Entendido
                </button>
              </motion.div>
            </>, document.body)}
