...
export default function PublicProfilePage() {
  ...
  const [stats, setStats] = useState<ProfileStats>({ postsCount: 0, friendsCount: 0, likesTotal: 0 });
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        const { data: profileData, error: profileError } = await supabase
          .from('profiles')
          .select('id, nick, age_group, avatar_snapshot_url, parent_approved')
          .eq('id', userId)
          .maybeSingle();
        ...
        setStats({ postsCount, friendsCount, likesTotal });
      } catch (err) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex flex-col items-center py-8 px-4">
        <AvatarBadge
          ...
          size="xl"
        />
        <h2 className="mt-4 text-xl font-bold">@{profile?.nick}</h2>
        <p className="text-muted-foreground text-sm mt-1">
          Grupo de edad: {profile?.age_group}
        </p>

        {/* Approval badge */}
        <div className={`mt-3 flex items-center gap-1.5 px-3 py-1 rounded-full text-sm ${
      ...
      <div className="flex justify-around py-6 border-t border-b mx-4">
        <div className="flex flex-col items-center">
          ...
          <span className="text-lg font-bold">{stats.postsCount}</span>
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-lg font-bold">{stats.likesTotal}</span>
