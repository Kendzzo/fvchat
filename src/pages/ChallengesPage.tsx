...
export default function ChallengesPage() {
  ...
  const handleSubmitEntry = async (contentUrl: string, visibility: "friends" | "public" = "public") => {
    const {
      error
    } = await submitEntry(contentUrl, visibility);
    if (error) {
  ...
  return <div className="min-h-screen bg-[#e8e6ff]">
      ...
      {todayChallenge && <ChallengeParticipateModal isOpen={showParticipate} onClose={() => setShowParticipate(false)} challengeId={todayChallenge.id} challengeTitle={todayChallenge.description} onSubmit={handleSubmitEntry} entriesRemaining={3 - (todayChallenge.my_entries_count || 0)} />}
