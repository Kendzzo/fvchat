...
export default function HomePage() {
  ...
  const friendsWithRecentPosts = friends
    .map((f) => {
      ...
      const mostRecentPost = friendPosts.sort(
        ...
        {activeTab === "posts" ? (
          ...
          >
            ...
            <div className="overflow-x-auto pb-2 scrollbar-hide mx-0 my-0 px-0 py-0 items-start justify-start flex flex-row gap-[15px] mt-[11px] mb-px">
              ...
              {friendsWithRecentPosts.map((friendship) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 flex-shrink-0 mx-[2px] transition-transform ${selectedFriendId === friendship.friend?.id ? "scale-105" : ""}`}
                >
                  <div
                    className={`avatar-frame w-16 h-16 px-0 py-0 ${selectedFriendId === friendship.friend?.id ? "ring-2 ring-secondary ring-offset-2" : ""}`}
                  >
                    <AvatarBadge
            ...
            {selectedFriend && (
              ...
              >
                <span className="text-sm font-medium text-primary">Viendo publicaciones de @{selectedFriend.nick}</span>
                <button
              ...
              filteredPosts.map((post, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-6 pt-2 mx-[20px] ml-[30px]">
                    ...
                    >
                      <Heart
                        className={`w-6 h-6 transition-colors ${post.isLiked ? "fill-destructive text-destructive" : "text-muted-foreground"}`}
                      />
          ...
          >
            {challengeLoading ? (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="px-3 py-1 rounded-full bg-warning/20 text-warning text-xs font-medium">Activo</div>
                </div>
                    ...
                    {(todayChallenge.top_entries || []).slice(0, 3).map((entry, i) => (
                      ...
                      >
                        {entry.user?.avatar_snapshot_url ? (
                          <img
                            src={entry.user.avatar_snapshot_url}
                            alt=""
              ...
              <div className="glass-card p-6 text-center">
                ...
                <p className="text-sm text-white">¡Vuelve mañana para el próximo desafío!</p>
              </div>
            )}
            ...
            <div className="glass-card p-6 text-center my-[30px]">
              ...
              <p className="text-sm text-muted-foreground">Cada día hay un nuevo desafío. ¡No te los pierdas!</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
