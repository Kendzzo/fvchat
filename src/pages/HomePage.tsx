...
export default function HomePage() {
  ...
  const friendsWithPosts = useMemo(() => {
    ...
    const friendPostMap = new Map<string, {
    ...
    return Array.from(friendPostMap.values()).sort((a, b) => b.latestPostDate.getTime() - a.latestPostDate.getTime()).slice(0, 10);
  }, [friends, posts]);
  ...
  return <div className="min-h-screen pb-20 bg-[#e9e7fd]">
      ...
      backgroundColor: "#e8e6ff"
        ...
        <div className="flex gap-5 min-w-max">
          ...
          {/* Friends Stories */}
          {friendsLoading ? <>
              {[1, 2, 3].map(i => <div key={i} className="flex flex-col items-center gap-2 min-w-[72px]">
                  <Skeleton className="w-[68px] h-[68px] rounded-full" />
                  <Skeleton className="w-12 h-3" />
                </div>)}
            </> : friendsWithPosts.length > 0 ? friendsWithPosts.map(({
          friend
        }, index) => friend && <button key={friend.id} onClick={() => setSelectedFriendId(selectedFriendId === friend.id ? null : friend.id)} className={`flex flex-col items-center gap-2 min-w-[72px] transition-opacity ${selectedFriendId && selectedFriendId !== friend.id ? "opacity-50" : ""}`}>
                    <div className={`w-[68px] h-[68px] rounded-full p-[3px] bg-gradient-to-br from-purple-400 via-purple-500 to-teal-400 ${selectedFriendId === friend.id ? "ring-2 ring-purple-600 ring-offset-2" : ""}`}>
                      <div className="w-full h-full rounded-full bg-white flex items-center justify-center overflow-hidden">
                        {friend.profile_photo_url ? <img src={friend.profile_photo_url} alt={friend.nick} className="w-full h-full object-cover" /> : <span className="text-2xl">{FRIEND_EMOJIS[index % FRIEND_EMOJIS.length]}</span>}
                      </div>
                    </div>
        ...
        <>
              {["Amigo1", "Amigo2", "Amigo3", "Amigo4"].map((name, i) => <div key={name} className="flex flex-col items-center gap-2 min-w-[72px]">
                  <div className="w-[68px] h-[68px] rounded-full p-[3px] bg-gradient-to-br from-purple-400 via-purple-500 to-teal-400">
                    <div className="w-full h-full rounded-full bg-white flex items-center justify-center">
      ...
      {/* Filter Banner */}
      {selectedFriend && <div className="bg-purple-100 px-4 py-2 flex items-center justify-between">
          <span className="text-sm text-purple-800">
            Viendo publicaciones de <strong>@{selectedFriend.nick}</strong>
          </span>
          <button onClick={() => setSelectedFriendId(null)} className="flex items-center gap-1 text-sm text-purple-600 hover:underline">
            <X className="w-4 h-4" />
            Quitar filtro
          </button>
        </div>}
      ...
      <div className="space-y-4 p-4 min-h-[60vh] pt-0 pb-0 bg-[#e9e7fd]">
        {postsLoading ? <>
          ...
          </> : filteredPosts.length === 0 ? <div className="text-center py-12 text-gray-500 bg-white/80 rounded-3xl">
              ...
              <div className="p-3 flex items-center gap-3 mb-0">
                <div className="p-0.5 rounded-full bg-gradient-to-br from-purple-600 via-pink-500 to-teal-400">
                  <div className="w-20 h-20 rounded-full bg-white p-0.5">
                    {post.author?.profile_photo_url ? <img src={post.author.profile_photo_url} alt={post.author?.nick} className="w-full h-full rounded-full object-cover" /> : <div className="w-full h-full rounded-full bg-purple-900 flex items-center justify-center">
                ...
                {(post.challenge_id || post.is_challenge_entry) && <button onClick={e => {
            e.stopPropagation();
            navigate("/app/challenges");
          }} className="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold bg-gradient-to-r from-amber-400 to-orange-500 text-white shadow-sm hover:opacity-90 active:scale-95 transition cursor-pointer" aria-label="Ir a Desafíos">
              ...
              {/* Post Content */}
              {post.content_url && (post.type === "photo" || post.type === "image") && <div className="relative">
                  <img src={post.content_url} alt="Publicación" className="w-full aspect-square object-cover" />
                  <PostStickerRenderer postId={post.id} />
                </div>}
              {post.content_url && post.type === "video" && <video src={post.content_url} controls className="w-full aspect-video bg-black" />}
              {post.text && !post.content_url && <div className="px-4 py-8 bg-gradient-to-br from-purple-100 to-teal-50">
              ...
              <div className="px-4 py-3">
                <div className="flex items-center gap-4">
                  {/* Like button */}
                  <button onClick={() => toggleLike(post.id)} className="flex items-center gap-1 text-gray-700 hover:text-red-500 transition-colors">
                    <Heart className={`w-6 h-6 ${post.isLiked ? "fill-red-500 text-red-500" : ""}`} />
                  </button>

                  {/* Comments button */}
                  <button onClick={() => setOpenCommentsPostId(prev => prev === post.id ? null : post.id)} className="flex items-center gap-1 text-gray-700 hover:text-purple-600 transition-colors">
                    <MessageCircle className="w-6 h-6" />
                  </button>
                </div>

                {/* Likes count */}
                {post.likes_count > 0 && <p className="mt-2 text-sm font-semibold text-gray-900">{post.likes_count} Me gusta</p>}

                {/* Comments count */}
                {post.comments_count && post.comments_count > 0 && <button onClick={() => setOpenCommentsPostId(prev => prev === post.id ? null : post.id)} className="text-sm text-gray-500 mt-1">
