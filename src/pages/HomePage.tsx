...
export default function HomePage() {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-purple-600/20 via-background to-background pb-20">
      {/* Header with Tabs */}
      <div className="bg-gradient-to-r from-purple-700 to-purple-500 px-4 pt-4 pb-0">
        <div className="flex">
          <button onClick={() => setActiveTab('publicaciones')} className={`flex-1 flex items-center justify-center gap-2 py-3 text-white font-semibold transition-all ${activeTab === 'publicaciones' ? 'opacity-100' : 'opacity-60'}`}>
        ...
        }} className={`flex-1 flex items-center justify-center gap-2 py-3 text-white font-semibold transition-all ${activeTab === 'desafios' ? 'opacity-100' : 'opacity-60'}`}>
        ...
        {/* Tab indicator */}
        <div className="flex">
          <div className={`h-1 transition-all duration-300 ${activeTab === 'publicaciones' ? 'w-1/2 bg-white rounded-full' : 'w-0'}`} />
        </div>
      </div>

      {/* Stories Row */}
      <div className="bg-gradient-to-b from-purple-200/80 to-purple-100/50 px-4 py-5 overflow-x-auto bg-purple-50">
        <div className="flex gap-4 min-w-max">
          {/* Tu historia */}
          <button onClick={() => navigate('/app/publish')} className="flex flex-col items-center gap-2 min-w-[80px]">
          ...
          {/* Friends Stories */}
          {friendsLoading ? <>
              {[1, 2, 3].map(i => <div key={i} className="flex flex-col items-center gap-2 min-w-[80px]">
                  <Skeleton className="w-20 h-20 rounded-full" />
                  <Skeleton className="w-16 h-4" />
                </div>)}
            </> : friendsWithPosts.length > 0 ? friendsWithPosts.map(({
          friend
        }, index) => friend && <button key={friend.id} onClick={() => setSelectedFriendId(selectedFriendId === friend.id ? null : friend.id)} className={`flex flex-col items-center gap-2 min-w-[80px] transition-opacity ${selectedFriendId && selectedFriendId !== friend.id ? 'opacity-50' : ''}`}>
                <div className={`w-20 h-20 rounded-full bg-white/90 flex items-center justify-center shadow-md ${selectedFriendId === friend.id ? 'ring-4 ring-primary' : ''}`}>
                  {friend.avatar_snapshot_url ? <img src={friend.avatar_snapshot_url} alt={friend.nick} className="w-full h-full rounded-full object-cover" /> : <span className="text-4xl">{FRIEND_EMOJIS[index % FRIEND_EMOJIS.length]}</span>}
                </div>
        ...
        <>
              {['Amigo1', 'Amigo2', 'Amigo3'].map((name, i) => <div key={name} className="flex flex-col items-center gap-2 min-w-[80px] opacity-50">
                  <div className="w-20 h-20 rounded-full bg-white/90 flex items-center justify-center shadow-md">
      ...
      {/* Filter Banner */}
      {selectedFriend && <div className="bg-primary/10 px-4 py-2 flex items-center justify-between">
          <span className="text-sm">
            Viendo publicaciones de <strong>@{selectedFriend.nick}</strong>
          </span>
          <button onClick={() => setSelectedFriendId(null)} className="flex items-center gap-1 text-sm text-primary hover:underline">
            <X className="w-4 h-4" />
            Quitar filtro
          </button>
        </div>}
      ...
      <div className="space-y-4 p-4 bg-purple-50">
        {postsLoading ? <>
            ...
            {selectedFriendId ? <p>Este amigo aún no tiene publicaciones</p> : <p>No hay publicaciones. ¡Sé el primero en publicar!</p>}
              ...
              {/* Post Content */}
              {post.content_url && (post.type === 'photo' || post.type === 'image') && <img src={post.content_url} alt="Publicación" className="w-full aspect-square object-cover" />}
              {post.content_url && post.type === 'video' && <video src={post.content_url} controls className="w-full aspect-video" />}
              {post.text && !post.content_url && <div className="px-4 py-6 bg-gradient-to-br from-purple-100 to-teal-50">
              ...
              <div className="p-4">
                <div className="flex items-center gap-6">
                  {/* Like button */}
                  <button onClick={() => toggleLike(post.id)} className="flex items-center gap-2 text-gray-600 hover:text-red-500 transition-colors">
                    <Heart className={`w-6 h-6 ${post.isLiked ? 'fill-red-500 text-red-500' : ''}`} />
                    <span className="font-medium">{post.likes_count}</span>
                  </button>

                  {/* Comments button */}
                  <button onClick={() => setOpenCommentsPostId(prev => prev === post.id ? null : post.id)} className="flex items-center gap-2 text-gray-600 hover:text-primary transition-colors">
